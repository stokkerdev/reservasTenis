...
class Space extends Model
{
    ...
    public function generateTimeBlocks(Carbon $date, int $intervalMinutes = 60): array
    {
        ...
        foreach ($availabilities as $availability) {
            ...
            while ($start->addMinutes($intervalMinutes)->lte($end)) {
                $blockStart = $start->copy()->subMinutes($intervalMinutes);
                $blockEnd = $start->copy();

                // Ensure the block doesn't exceed the availability end time
                if ($blockEnd->gt($end)) {
                    break;
                }
